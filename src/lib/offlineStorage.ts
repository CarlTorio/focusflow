    ...
    for (const m of mutations) {
      try {
        ...
        if (m.operation === "insert") {
          const res = await supabase.from(m.table).insert(m.data);
          error = res.error;
        } else if (m.operation === "update") {
          const res = await supabase.from(m.table).update(m.data).eq(m.matchColumn!, m.matchValue!);
          error = res.error;
        } else if (m.operation === "delete") {
          const res = await supabase.from(m.table).delete().eq(m.matchColumn!, m.matchValue!);
          error = res.error;
        }
