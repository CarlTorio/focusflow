...
export default function Planner() {
  ...
  const { schedules, isLoading, completeSchedule, createPlannerTask } = usePlanner(dateRange.startStr, dateRange.endStr);
  ...
  const headerText = useMemo(() => {
    if (viewMode === "columns") {
      if (isMobile) {
        return format(selectedMobileDay, "MMMM d, yyyy");
      }
      return `${format(baseDate, "MMMM d, yyyy")} - ${format(addDays(baseDate, 1), "MMMM d, yyyy")}`;
    } else {
      const start = startOfWeek(baseDate, { weekStartsOn: 1 });
      const end = endOfWeek(baseDate, { weekStartsOn: 1 });
  ...
  // Mobile day bar for columns view
  ...
  const handleSave = (input: Parameters<typeof createPlannerTask.mutate>[0]) => {
    createPlannerTask.mutate(input);
    setModalOpen(false);
  };
  ...
  return (
    <div className="pb-20 md:pb-8">
      ...
      <div className="mx-auto max-w-6xl px-4 py-4">
        ...
        <div className="mb-5 flex items-center gap-3 flex-wrap">
          ...
          <div className="flex rounded-xl border border-border p-1 bg-muted/30">
            <button
              ...
              className={cn(
                ...
                viewMode === "columns" ? "bg-card shadow-sm text-foreground" : "text-muted-foreground"
              )}
            >
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full bg-foreground text-background transition-transform hover:scale-105"
          >
                    ...
                    >
                      ...
                      <span
                        className={cn(
                          ...
                          selected ? "bg-foreground text-background" : current ? "bg-primary-light text-primary" : "text-foreground"
                        )}
                      >
                        {format(day, "d")}
                      </span>
            ...
            <div className={cn("flex gap-6", isMobile && "flex-col")}>
              {isMobile ? (
                <DayColumn
                  ...
                  onAddAnytime={() => openAddModal(selectedMobileDay)}
                  onAddPlanned={() => openAddModal(selectedMobileDay, `${String(new Date().getHours() + 1).padStart(2, "0")}:00`)}
                />
              ) : (
                <>
                  <DayColumn
                    ...
                    onComplete={id => completeSchedule.mutate(id)}
                    onAddAnytime={() => openAddModal(baseDate)}
                    onAddPlanned={() => openAddModal(baseDate, `${String(new Date().getHours() + 1).padStart(2, "0")}:00`)}
                  />
                  ...
                  <DayColumn
                    ...
                    onComplete={id => completeSchedule.mutate(id)}
                    onAddAnytime={() => openAddModal(addDays(baseDate, 1))}
                    onAddPlanned={() => openAddModal(addDays(baseDate, 1), `${String(new Date().getHours() + 1).padStart(2, "0")}:00`)}
                  />
                </>
              )}
            </div>
      ...
      <AddPlannerModal
        ...
        isSaving={createPlannerTask.isPending}
      />
    </div>
  );
}
